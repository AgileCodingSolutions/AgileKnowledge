...
export class ChatApplicationService {
    ...
    /**
     * @param chatDialohId (optional) 
     * @return Success
     */
    deleteChatDialogHistory(chatDialohId: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/ChatApplication/DeleteChatDialogHistory?";
        if (chatDialohId === null)
            throw new Error("The parameter 'chatDialohId' cannot be null.");
        else if (chatDialohId !== undefined)
            url_ += "chatDialohId=" + encodeURIComponent("" + chatDialohId) + "&";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processDeleteChatDialogHistory(_response);
        });
    }

    protected processDeleteChatDialogHistory(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class KnowledgeService {
     ...
     * @return Success
     */
    getDetailVectorQuantity(knowledgeBaseDetailsId: string | undefined, filter: string | undefined, sorting: string | undefined, pageNumber: number | undefined, pageSize: number | undefined): Promise<KnowledgeBaseDetailVectorQuantityDtoPagedResultDto> {
